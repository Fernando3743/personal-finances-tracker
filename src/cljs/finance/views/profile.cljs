...
(defn profile-edit-section []
  (let [user @(rf/subscribe [:auth/user])
        ...
        form @(rf/subscribe [:profile/edit-form])
        name-val (:name form)
        email-val (:email form)
      ...
      [:div {:class "grid grid-cols-1 md:grid-cols-2 gap-4"}
       [:div {:class "space-y-1.5"}
        ...
        [:input {:class (str "w-full h-11 px-4 rounded-lg border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 text-neutral-900 dark:text-neutral-50 "
                 ...
                 :type "text"
                 :value name-val
                 :on-change #(rf/dispatch [:profile/update-edit-form :name (-> % .-target .-value)])}]]
       [:div {:class "space-y-1.5"}
        ...
        [:input {:class (str "w-full h-11 px-4 rounded-lg border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 text-neutral-900 dark:text-neutral-50 "
                 ...
                 :type "email"
                 :value email-val
                 :on-change #(rf/dispatch [:profile/update-edit-form :email (-> % .-target .-value)])}]]]
...
(defn password-change-section []
  (let [saving? @(rf/subscribe [:profile/saving?])
        form @(rf/subscribe [:profile/password-form])
        current-pw (:current-password form)
        new-pw (:new-password form)
        confirm-pw (:confirm-password form)
        passwords-match? (= new-pw confirm-pw)
     ...
     [:div {:class "space-y-4"}
      [:div {:class "space-y-1.5"}
       ...
       [:input {:class (str "w-full h-11 px-4 rounded-lg border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 text-neutral-900 dark:text-neutral-50 "
                ...
                :on-change #(rf/dispatch [:profile/update-password-form :current-password (-> % .-target .-value)])}]]
      [:div {:class "grid grid-cols-1 md:grid-cols-2 gap-4"}
       [:div {:class "space-y-1.5"}
        ...
        [:input {:class (str "w-full h-11 px-4 rounded-lg border bg-white dark:bg-neutral-900 text-neutral-900 dark:text-neutral-50 "
                 ...
                 :on-change #(rf/dispatch [:profile/update-password-form :new-password (-> % .-target .-value)])}]
       ...
       [:div {:class "space-y-1.5"}
        ...
        [:input {:class (str "w-full h-11 px-4 rounded-lg border bg-white dark:bg-neutral-900 text-neutral-900 dark:text-neutral-50 "
                 ...
                 :on-change #(rf/dispatch [:profile/update-password-form :confirm-password (-> % .-target .-value)])}]
                 ...
                 :on-click #(do
                              (rf/dispatch [:profile/change-password
                                            {:current_password current-pw
                                             :new_password new-pw}])
                              (rf/dispatch [:profile/reset-password-form]))}
...
(defn danger-zone-section []
  (let [deleting? @(rf/subscribe [:profile/deleting?])
        delete-form @(rf/subscribe [:profile/delete-form])
        show-delete-modal? (:show-modal? delete-form)
        delete-password (:password delete-form)]
       ...
       [:div {:class "space-y-3"}
        ...
        [:div {:class "flex items-center justify-between p-4 bg-red-100/30 dark:bg-red-900/20 rounded-lg"}
         ...
         [:button {:class "inline-flex items-center gap-2 px-3 py-2 rounded-lg bg-red-500 text-white hover:bg-red-600 transition-colors"
                   :on-click #(rf/dispatch [:profile/toggle-delete-modal true])}
          [icon :trash {:width 16 :height 16}]
          [:span "Delete Account"]]]]

       (when show-delete-modal?
       [:div {:class "fixed inset-0 bg-black/50 z-modal flex items-center justify-center p-4"
              :on-click #(rf/dispatch [:profile/toggle-delete-modal false])}
          [:div {:class "bg-white dark:bg-neutral-800 rounded-xl border border-neutral-200 dark:border-neutral-700 w-full max-w-md p-6"
           ...
           [:div {:class "space-y-1.5 mb-4"}
            ...
            [:input {:class (str "w-full h-11 px-4 rounded-lg border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 text-neutral-900 dark:text-neutral-50 "
                 ...
                 :on-change #(rf/dispatch [:profile/update-delete-form :password (-> % .-target .-value)])}]]
  ...
  (r/create-class
   {:component-did-mount
    (fn [_]
      ...
      (rf/dispatch [:profile/init-edit-form]))
