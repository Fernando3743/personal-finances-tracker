  ...
  (let [filter-category @(rf/subscribe [:expenses/filter-category])
    ...
    [:div {:class "flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6"}
     ...
     [:div {:class "flex items-center gap-3"}
      [:span {:class "text-sm text-neutral-600 dark:text-neutral-400"} (str expense-count " expenses")]
      (when has-filters?
  ...
  (let [curr (or currency :COP)]
    [:div {:class "bg-white dark:bg-neutral-800 rounded-xl border border-neutral-200 dark:border-neutral-700 p-4"}
     [:div {:class "flex items-center gap-3"}
      ...
      [:div {:class "font-mono font-semibold text-red-600 dark:text-red-500"}
       (currency/format-currency (- amount) curr {:show-sign? true})]]
  ...
  (let [breakdown @(rf/subscribe [:expenses/category-breakdown])]
    (when (seq breakdown)
      [:div {:class "bg-white dark:bg-neutral-800 rounded-xl border border-neutral-200 dark:border-neutral-700 p-5 mb-6"}
       [:h4 {:class "text-sm font-semibold text-neutral-900 dark:text-neutral-50 mb-4"} "Spending by Category"]
       [:div {:class "space-y-3"}
        (for [{:keys [category amount percentage]} (take 5 breakdown)]
          ...
          [:div {:class "flex items-center gap-3"}
           ...
           [:span {:class "text-xs text-neutral-600 dark:text-neutral-400 w-10 text-right"} (str (.toFixed percentage 0) "%")]])]])))
...
(defn expenses-view []
  (let [expenses @(rf/subscribe [:expenses/filtered-list])
    ...
    [:div {:class "space-y-6"}
     [:div {:class "mb-6"}
      [:h1 {:class "text-2xl font-bold text-neutral-900 dark:text-neutral-50"} "Expenses"]
      [:p {:class "text-neutral-600 dark:text-neutral-400 text-sm mt-1"} "Track all your spending"]]
     ...
     (cond
       ...
       [:div {:class "text-center py-16 bg-white dark:bg-neutral-800 rounded-xl border border-neutral-200 dark:border-neutral-700"}
        ...
        [:h3 {:class "text-lg font-semibold text-neutral-900 dark:text-neutral-50 mb-2"} "No expenses found"]
